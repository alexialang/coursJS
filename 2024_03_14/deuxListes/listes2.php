...
try {
  ...
  $resultat = $dbh->prepare("SELECT * FROM villes_france_free INNER JOIN departement ON villes_france_free.ville_departement = departement.departement_code WHERE departement_nom = :departement");
  $resultat->execute(['departement' => $_GET['departement']]);
